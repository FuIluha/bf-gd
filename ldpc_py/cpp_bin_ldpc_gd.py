"""ctypes wrapper for the C++ exact-gradient decoder."""
...
LIBRARY_PATH = Path(tempfile.gettempdir()) / f"bf_gd_cpp_gd_{SOURCE_HASH}.so"
INVALID_RESULT = np.iinfo(np.uint32).max


def lib_compile():
    """Compile the C++ exact-gradient decoder shared library."""
...
def load_library():
    ...
    library.cpp_gd_create.restype = ctypes.c_void_p
    library.cpp_gd_create.argtypes = [
        ctypes.c_uint32,
        ctypes.c_uint32,
        ctypes.c_uint32,
        ctypes.c_double,
    ...
    library.cpp_gd_decode_float32.restype = ctypes.c_uint32
    library.cpp_gd_decode_float32.argtypes = [
        ctypes.c_void_p,
        float32_array,
        float32_array,
    ]
    library.cpp_gd_decode_float64.restype = ctypes.c_uint32
    library.cpp_gd_decode_float64.argtypes = [
        ctypes.c_void_p,
        float64_array,
        float64_array,
    ]
    library.cpp_gd_free.restype = None
    library.cpp_gd_free.argtypes = [ctypes.c_void_p]
    return library


class CppBinLdpcGdDecoder(BinLdpcDecoderBase):
    """C++ decoder using the exact LLR-domain objective gradient."""
    ...
    def __init__(self, alist_filename, **kwargs):
        ...
        self._decoder = self._library.cpp_gd_create(
            self.block_length,
            self.n_checks,
            self.n_iterations,
            self.learning_rate,
        ...
        if not self._decoder:
            raise RuntimeError("Failed to create C++ gradient decoder")
    ...
    def decode(self, llr_in, llr_out, rng=None):
        ...
        if llr_in.dtype == np.float32:
            result = self._library.cpp_gd_decode_float32(
                self._decoder,
                llr_in,
                llr_out,
            )
        elif llr_in.dtype == np.float64:
            result = self._library.cpp_gd_decode_float64(
                self._decoder,
                llr_in,
                llr_out,
            )
            ...
            raise TypeError(
                "C++ gradient decoder supports only float32 and float64 LLRs"
            )

        if result == INVALID_RESULT:
            raise FloatingPointError(
                "gradient decoder state exceeded the output floating-point range"
            )
        return result
