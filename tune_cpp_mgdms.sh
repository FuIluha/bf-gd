#!/usr/bin/env bash

# Submit the C++ MGDMS parameter search to Slurm.

#SBATCH --job-name=cpp-mgdms-tune
#SBATCH --output=logs/tune_cpp_mgdms_%j.out
#SBATCH --error=logs/tune_cpp_mgdms_%j.err
#SBATCH --partition=amd
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=128
#SBATCH --mem=32G
#SBATCH --time=24:00:00
#SBATCH --mail-type=BEGIN,END,FAIL

set -euo pipefail

if [[ -n "${SLURM_SUBMIT_DIR:-}" ]]; then
    project_dir=$SLURM_SUBMIT_DIR
else
    project_dir=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
fi

cd "$project_dir"
mkdir -p logs

python_bin="$project_dir/.venv/bin/python3"
if [[ ! -x "$python_bin" ]] || ! "$python_bin" --version >/dev/null 2>&1; then
    echo "Working Python interpreter not found: $python_bin" >&2
    echo "Create .venv and install requirements before submitting this job." >&2
    exit 1
fi

if [[ "$*" != *"--full-grid"* ]] && [[ "$*" != *"--momentum-only"* ]]; then
    set -- --momentum-only "$@"
fi

exec srun "$python_bin" tune_cpp_mgdms.py \
    --config experiments/experiment_cpp_mgdms.json \
    --workers "${SLURM_CPUS_PER_TASK:-128}" \
    "$@"
