"""Grid-search the C++ exact-gradient decoder parameters at one SNR point."""
...
DEFAULT_CONFIG = PROJECT_DIR / "experiments" / "experiment_cpp_gd.json"
DEFAULT_OUTPUT = PROJECT_DIR / "params_cpp_gd.txt"

DEFAULT_LEARNING_RATES = (0.02, 0.05, 0.1, 0.2, 0.4, 0.8)
DEFAULT_REGULARIZATIONS = (0.25, 0.5, 0.75, 1.0, 1.5, 2.0)
DEFAULT_ALPHAS = (0.25, 0.5, 0.75, 1.0, 1.5, 2.0)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Search C++ exact-gradient decoder hyperparameters using FER at "
            "a fixed SNR. Every new best result is saved immediately."
        )
    )
...
def load_base_experiment(config_path):
    ...
    if experiment["codec"].get("algorithm") != (
        "cpp gradient descent decoder"
    ):
        raise ValueError(
            "the selected config must use the C++ gradient decoder"
        )
...
def parameter_grid(args, base_params):
    ...
    candidates = [baseline]
    for learning_rate, regularization, alpha in itertools.product(
        args.learning_rates,
