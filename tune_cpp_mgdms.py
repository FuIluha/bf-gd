"""Grid-search the C++ momentum gradient-descent min-sum parameters at one SNR point."""
...
DEFAULT_LEARNING_RATES = tuple(np.round(np.arange(0.2, 1.0 + 1e-9, 0.2), 6))
DEFAULT_LEARNING_RATE_DECAYS = (0.0, 0.01, 0.03, 0.05, 0.1)
DEFAULT_ALPHAS = tuple(np.round(np.arange(0.25, 3.0 + 1e-9, 0.25), 6))
DEFAULT_L2 = (0.0,0.6, 1.0, 1.2)
DEFAULT_MOMENTUM_VALUES = tuple(np.round(np.arange(0.0, 0.96 + 1e-9, 0.04), 6))
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Search C++ MGDMS hyperparameters using FER at a fixed SNR. "
...
def load_base_experiment(config_path):
    ...
    if experiment["codec"].get("algorithm") != "cpp momentum gradient descent min-sum":
        raise ValueError("the selected config must use the C++ momentum GDMS decoder")
...
def main():
    ...
    print(
        f"CPP MGDMS search: SNR={args.snr:g} dB, "
